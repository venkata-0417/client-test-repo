...
export class FileUploadComponent implements OnInit {
  ...
  OnBack() {
    this.router.navigate(['/process/pull-code']);
  }
  ...
  prepareTableData(tableData, headerList) {
    ...
    let tableHeader: any = [
      ...
      { field: 'size', header: 'File Size (MB)', index: 2 },
  ...
  convertToMB(bytes) {
    const mbSize = bytes / (1024 * 1024); // Convert bytes to MB
    return mbSize.toFixed(2) + ' MB'; // Round to 2 decimal places
  }
